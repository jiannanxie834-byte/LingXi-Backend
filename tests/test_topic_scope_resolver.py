...
class TopicScopeResolverTest(unittest.TestCase):
    def test_specific_lstm_question_stays_concept(self):
        ...
        self.assertEqual(result["display_topic"], "LSTM")
        self.assertEqual(result["primary_unit_id"], "dl_rnn_lstm_gru")
    ...
    def test_full_chapter_marker_allows_chapter(self):
        ...
        self.assertEqual(result["display_topic"], "RNN、LSTM、GRU")
    ...
    def test_course_level_request_does_not_expand_all_chapters(self):
        ...
        self.assertEqual(result["display_topic"], "深度学习")
        self.assertFalse(result["should_generate_full_chapter"])
        self.assertEqual(result["expansion_policy"], "course_orientation_diagnosis_path_only")
    ...
    def test_project_request_is_project_scope(self):
        ...
        self.assertEqual(result["display_topic"], "CNN")
        self.assertEqual(result["primary_unit_id"], "dl_cnn_conv_basic")
...
class ResourceAgentScopeTitleTest(unittest.TestCase):
    def test_resource_title_prefers_display_topic(self):
        ...
        self.assertTrue(all(item["title"].startswith("LSTM ·") for item in plan["resources"]))
        self.assertTrue(all(item["unit_title"] == "LSTM" for item in plan["resources"]))
