...
CHAPTER_DIR = COURSE_DIR / "courseware" / "chapter_12_algorithm_project"
CHAPTER_ID = "chapter_12_algorithm_project"
...
class DsaChapter12ResourcesTest(unittest.TestCase):
    def test_chapter_12_has_required_six_sections(self):
        expected_files = [
            "01_project_overview.md",
            "02_algorithm_visualizer.md",
            "03_maze_path_finder.md",
            "04_online_judge_trainer.md",
            "05_project_report.md",
            "06_presentation_and_reflection.md",
        ]
    ...
    def test_chapter_12_project_resources_are_complete(self):
        self.assertTrue((CHAPTER_DIR / "resources" / "chapter_overview.md").exists())
        self.assertTrue((CHAPTER_DIR / "resources" / "project_brief.md").exists())
        self.assertTrue((CHAPTER_DIR / "resources" / "rubric.md").exists())
        self.assertTrue((CHAPTER_DIR / "resources" / "report_template.md").exists())
        ...
        self.assertGreaterEqual(_compact_len(CHAPTER_DIR / "resources" / "project_brief.md"), 1200)
        self.assertLessEqual(_compact_len(CHAPTER_DIR / "resources" / "project_brief.md"), 1800)

        rubric = (CHAPTER_DIR / "resources" / "rubric.md").read_text(encoding="utf-8")
        self.assertEqual(len(re.findall(r"^## \d+\.", rubric, flags=re.MULTILINE)), 5)
        for label in ["优秀", "良好", "合格", "需改进"]:
            self.assertEqual(rubric.count(label), 5)

        report = (CHAPTER_DIR / "resources" / "report_template.md").read_text(encoding="utf-8")
        self.assertEqual(len(re.findall(r"^## \d+\.", report, flags=re.MULTILINE)), 9)

    def test_chapter_12_banks_and_video_policy(self):
    ...
    def test_chapter_12_global_indexes_are_aligned(self):
        ...
        self.assertEqual(manifest["stage"], "curated_base_resource")
        ...
        self.assertEqual(chapter["title"], "第 12 章 综合项目：算法可视化与刷题训练系统")
        self.assertEqual(len(chapter["sections"]), 6)
        self.assertEqual(chapter["sections"][0]["section_id"], "sec_12_project_overview")
        self.assertEqual(chapter["sections"][5]["section_id"], "sec_12_presentation_reflection")
        ...
        self.assertEqual(len(entry["primary_resources"]), 6)
        ...
        for unit_id in [
            "dsa_project_overview",
            "dsa_project_algorithm_visualizer",
            "dsa_project_maze_path_finder",
            "dsa_project_online_judge_trainer",
            "dsa_project_report_template",
            "dsa_project_rubric",
            "dsa_project_testing",
            "dsa_project_presentation",
            "dsa_project_reflection",
        ]:
            self.assertIn(unit_id, unit_ids)

    def test_chapter_12_has_no_forbidden_references(self):
        forbidden = [
            "TODO",
            "LeetCode",
            "main_note.md",
