...
def _validate_resource_plan(plan):
    ...
    for index, resource in enumerate(resources, start=1):
        ...
        if not resource.get("unit_id"):
            raise RuntimeError(f"资源规划第 {index} 项必须绑定数据结构与算法知识单元 unit_id")
        ...
        "resources": [
            {
                ...
                "course_id": "",
                "chapter_id": "",
                "unit_id": (dsa_course_map_service.get_intro_unit() or {}).get("unit_id", ""),
...
def _title_for_resource(topic, resource_type, course_match):
    display_topic = course_match.get("display_topic") or topic or course_match.get("normalized_topic") or "算法学习主题"
    return f"{display_topic} · {resource_type}"


def _summary_for_resource(resource_type, course_match, intent):
    unit_title = course_match.get("display_topic") or course_match.get("normalized_topic") or course_match.get("topic") or "数据结构与算法知识点"
    ...
    summary_map = {
        ...
        artifact_types.CODE_LAB: f"提供与「{unit_title}」匹配的算法代码实验指导、代码骨架、运行方式和调试任务。",
        ...
        artifact_types.INTERACTIVE_ANIMATION: f"生成「{unit_title}」交互动画规格，用于前端渲染排序、查找、递归、树图或 DP 过程。",
        artifact_types.ANIMATION_STORYBOARD: f"生成「{unit_title}」动画分镜，便于 PPT、演示视频和可视化说明使用。",
        artifact_types.PROJECT_BRIEF: f"围绕「{unit_title}」设计算法课程项目任务书、验收标准和评分 Rubric。",
...
def _build_resource_item(topic, intent, resource_type, plan_title, profile, semantic_result, subject_category, generation_context, course_match):
    ...
    return {
        ...
        "course_id": course_match.get("course_id") or dsa_course_map_service.COURSE_ID,
        "chapter_id": course_match.get("chapter_id") or unit.get("chapter_id") or "",
        "section_id": course_match.get("section_id") or unit.get("section_id") or semantic_result.get("section_id") or "",
        "chapter": course_match.get("chapter") or "",
        "unit_id": course_match.get("unit_id") or unit.get("unit_id") or "",
        "unit_ids": course_match.get("unit_ids") or semantic_result.get("unit_ids") or [course_match.get("unit_id") or unit.get("unit_id") or ""],
