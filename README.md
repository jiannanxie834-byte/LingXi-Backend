...
- 学习主流程：学生自然语言输入 → 六项核心学习画像 → 课程知识检索 → 5 步学习路径 → 5 类个性化资源 → 教师审核 → 练习评价与画像更新
- 画像兼容：学生端展示当前知识水平、学习目标、练习表现、薄弱知识点、路径执行和资源偏好；后端保留原十维快照用于兼容既有事件和历史数据
...
`lingxi.db` 是早期 SQLite 本地测试文件，当前提交版使用独立的脱敏基线数据库。
...
配置有效的讯飞星火访问凭证后，可使用下面的输入检查完整生成流程：

`我现在只是数据结构与算法的初学者，我现在想学习动态规划`

预期结果：系统定位到第 10 章动态规划，更新六项核心学习画像，调用讯飞星火生成学习路径和主要内容，并结合课程知识库整理多类个性化资源，再执行质量检查、持久化和管理员审核。生成内容与耗时会受模型服务和网络状态影响。
