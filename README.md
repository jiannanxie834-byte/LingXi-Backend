...
- 画像兼容：学生端展示当前知识水平、学习目标、练习表现、薄弱知识点、路径执行和资源偏好；后端保留原十维快照用于兼容既有事件和演示数据
...
预期结果：定位到第 10 章动态规划，更新六项核心学习画像，生成 5 步路径和五类资源，并进入审核队列。该固定用例回放已经审核的动态规划 Artifact，同时完整复用生成任务、阶段进度、质量检查、落库和审核流程。
