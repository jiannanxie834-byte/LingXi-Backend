# 第 9 章 Scaled Dot-Product Attention 实验

## 1. 实验目标
围绕「Scaled Dot-Product Attention、Multi-Head Attention、位置编码、Encoder 和 Mask」完成一个可运行实验，要求学生能解释输入 shape、模型输出、损失变化、关键超参数和实验结论之间的关系。
...
## 4. 完整代码
将下面代码保存为 `chapter_09_attention_demo.py`。
...
## 5. 运行命令
```bash
python chapter_09_attention_demo.py
```
如果在 notebook 中运行，请先确认当前内核已安装 torch，并逐单元执行完整代码。

## 6. 关键代码解释
- 数据构造部分负责控制输入维度、标签规则和 batch 组织方式。
- 模型结构部分体现本章主题：Scaled Dot-Product Attention、Multi-Head Attention、位置编码、Encoder 和 Mask。
...
4. 用 150 字解释该实验如何帮助理解「Scaled Dot-Product Attention、Multi-Head Attention、位置编码、Encoder 和 Mask」。
